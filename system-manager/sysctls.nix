...
# distro on their own).
#
# Rendered as a single `environment.etc."sysctl.d/60-nixram.conf"` file
...
in
{
  config = mkIf (cfg.enable && cfg.sysctls.enable) {
    environment.etc."sysctl.d/60-nixram.conf" = {
    ...
    systemd.services.nixram-sysctl-reapply = {
      description = "Re-apply /etc/sysctl.d/60-nixram.conf after a declarative change (systemd-sysctl is boot-only)";
      wantedBy = [ "multi-user.target" ];
      restartTriggers = [ config.environment.etc."sysctl.d/60-nixram.conf".source ];
      serviceConfig = {
        Type = "oneshot";
